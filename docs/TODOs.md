...
- [ ] 增强日志（语句级）
    - [ ] 为 Move/StepMove 的关键调用加 BEFORE/AFTER breadcrumb：
        ...
        - wait_until_done
        - stop
    - [ ] 新建并统一写入 `crash_breadcrumb.log`，确保每条日志 flush。
- [ ] 崩溃证据归档
...
## 第三步：最小修复（按风险优先级）
### 3.1 线程安全修复（高优先级）
- [ ] 检查并消除 DO 工作线程对 UI 控件的直接 `setValue`。
- [ ] 所有 UI 更新通过 Qt signal/slot 切回主线程。

### 3.2 Move/StepMove 时序修复（高优先级）
- [ ] `StepMove` 内先计算目标值 `target`。
- [ ] `Move` 支持显式参数 `target_pos`，优先使用该值计算 `distance`。
...
- [ ] samps_per_chan > 0
- [ ] rate 在设备允许区间内
- [ ] 非法参数直接中止下发，记录 `DO_PARAM_INVALID`。
